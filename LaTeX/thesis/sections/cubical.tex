...
Despite a powerful interpretation, HoTT lacks good computational properites.
In particular, Univalence being an axiom breaks computational canonicity and normalization is not known to hold.~\cite{HoTT}
This is troublesome for theorem proving applications, motivating a computationally sound framework instead. 

HoTT affords a standard presheaf model in simplicial sets.~\cite{kapulkin2021simplicial}  
Simplices offer poor constructive behaviour.~\cite{bezem2015non} This motivates a model in cubical sets instead, which being described by products instead of joins, are 
nicer combinatorially. We shall describe cubical sets and their computationally preferable behaviour in type theory. 
...
\end{definition}
We will call morphisms \emph{substitutions} as they encode substituting a name to ${0,1}$. 
