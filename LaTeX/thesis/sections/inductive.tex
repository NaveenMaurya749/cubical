...
Type Theory provides deductive rules to give schemas to construct, manipulate, and
eliminate various constructions. A number of these
constructions are enforced by giving dedicated rules in the deductive system, known as \textit{type formers}.
Type Theories also allow the user to define their own constructions known as inductive types,
with a similar schema.

The general scheme for these type formers or inductive types is as follows. An type former is defined by:
\begin{itemize}
...
\item \textbf{\textit{Elimination rules}} to give a recipe how to use/consume terms of the type. 
    ...
    Both work by reducing the task to define the function on a few special cases, given by the introduction rules.
\item A list of \textbf{\textit{computation rules}}, dictating how the elimination acts on the constructors. 
\item (Optional) \textbf{\textit{uniqueness principles}}, dictating how each term of the type
  is uniquely determined by the result of elimination rules applied to it.
\end{itemize}

Here is a succinct, informal description of these type formmers in Type Theory. They may also be interpretated as logical objects 
under `propositions as types' interpretation. A detailed, formal description
is avaiable in Appendix I of \cite{HoTT}.

\subsection{Essential Type Formers/Inductives}
...
We get $f(a) : B$ for any $f : A \to B$ and $a : A$.

Iterated function types such as $A \to (B \to C))$ are effectively multivarible function types (due to Product-Exponential adjunction),
so the parentheses are omitted and associated from right by default, like $A \to B \to C$. This is called \emph{currying}.
Currying is also used in lambda notation, writing $(\lambda x.\lambda y.\Phi)$ instead of $(\lambda x.(\lambda y.\Phi))$.

Logically, $A \to B$ correponds to $A \implies B$, albeit constructively. That is, to show $A \implies B$, one must construct
an explicit function taking terms of type $A$ to terms of type $B$.

\subsubsection{Dependent Function Types ($\Pi$-types)}
Given $A : \U, B : A \to \U$, we can form the type of dependent functions $\Prod{x:A} B(x) : \U$. Its terms are functions with
codomain dependent on the input from the domain. They follow similar rules as regular function types, but $f(a) : B(a)$
for any $a : A$ and  $f : \Prod{a:A}{B(a)}$.

Logically, $\Prod{x:A}{B(x)}$ corresponds to the predicate $\forall x \in A, (B(x))$ and can be read as such, except it needs to be constructive. 

\subsubsection{Product Types}
Given $A, B : \U$, we can form the product type $A \times B : \U$, term of which are pairs $(a, b)$, given by $a : A, b : B$. 
...
Given $A, B : \U$, we can form the coproduct type $A + B : \U$ given by two constructors $\inl : A \to A + B$, and $\inr : B \to A + B$.
...
This is the dependent version of a product type. Given $A : \U, B : A \to \U$, we can form $\Sum{a:A}{B(a)} : \U$.
Say $a : A, b : B(a)$, then there is a term $(a,b) : \Sum{a:A}{B(a)}$. 
...
Logically, $\Sum{x:A}{B(x)}$ corresponds to the predicate $\exists x \in A, (B(x))$, and may be read as such barring constructivity.
...
However, $\2$ does not have any equivalent in first-order logic. In can be shown that MLTT has a richer logic than first-order under this
correspondence.~\cite{HoTT}
...
(This is the usual principle of mathematical induction!)

\subsubsection{Identity Types}
Given $A : \U$, we can form the identity type family $\Id_A : A \to A \to \U$. Given $x, y: A$, we have $\Id_A(x,y)$, also denoted as
$x =_A y$, which is the type of all \textit{propositional equalities} between $x$ and $y$. The introduction rule is by using
$\refl : \Prod{a:A} (a =_A a)$.
Identities being types themselves, we can form iterated identity types such as $\Id_{\Id_A(x,y)}(p,q)$ of eqaulities between
$p, q : x =_A y$. Intensional MLTT has \emph{proof relevance} meaning one allows multiple proofs of identity types to not be equal,
making it sensical to question about iterated identity types.
This way, proofs themselves become mathematical objects, susceptible to analysis and proof.

The induction principle for Identity Types is called \textbf{path induction}, which says 
given $C : \Prod{x, y : A} (x =_A y) \to \U$ and a function $c : \Prod{x : A}{C(x,x,\refl_x)},$
there is a function $f : \Prod{x,y : A}{\Prod{p: x =_A y} C(x,y,p)}$ such that $f(x,x,\refl_x) :\deq c(x)$.
That is, to define a function on all equalities between terms in $A$, it suffices to exhibit a function on all
reflexive reflexive equalities $\refl_x$ over all terms $x : A$.
...
There is a complete formalism of inductive types up to certain demands for the constructors, first appearing in~\cite{martin1982constructive}.
